$(document).ready(function () {
    console.log('hola desde ivancho rancho');
    calculartotal();
...
function cuandosecambiaproducto() {
    ...
    var $cantidaddeproducto = $('#detallecompra_set-group > div > fieldset > table > tbody').children('tr').length;

    // aqui en vectornombre cargo los id de los elementos del detalle producto
    vectornombre = [];
    vectornombreid = [];
    for (var i = 0; i < $cantidaddeproducto - 2; i++) {
        var nombredeproductoconid = '#select2-id_detallecompra_set-' + i + '-producto-container';
    ...
    for (var i = 0; i < $cantidaddeproducto - 2; i++) {
        var cajapreciounitario = '#id_detallecompra_set-' + i + '-preciounitario';
    ...
    if(posiciones.length != 0){
        $.ajax({
            ...
            success: function (response){
                ...
                // #id_detallecompra_set-0-preciounitario
                var cajapreciounitario;
                for(var i=0;i <= vectoridp.length;i++) {
                    var cajapreciounitario = '#id_detallecompra_set-'+posiciones[i]+'-preciounitario';
                    $(cajapreciounitario).val('');
                }
                for(var i=0;i < vectoridp.length;i++) {
                    var cajapreciounitario = '#id_detallecompra_set-'+posiciones[i]+'-preciounitario';
...
function calcularsubtotales() {
    ...
    for (var i=0 ; i<$cantidaddeproducto-2 ; i++){
        var preciouni = '#id_detallecompra_set-'+i+'-preciounitario';
        var cantidad = '#id_detallecompra_set-'+i+'-cantidad';
        var subtotald = '#id_detallecompra_set-'+i+'-subtotal';
        ...
        // var ivaf = '#id_detallecompra_set-'+i+'-subtotaliva';
...
function calculartotal() {
    ...
    if($cantidaddeproducto-2 > 0){
        for(i = 0; i<$cantidaddeproducto-2;i++){
            a1 = '#id_detallecompra_set-'+i+'-subtotal';
            console.log($(a1).val());
            console.log('$(a1).val()');
            console.log( '');
